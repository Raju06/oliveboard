...
def arrangementInd(clist):
	type_of_arrangement=np.random.choice([1,2],1)
...
def createlist(iteration,Number_of_sets,Type_of_subset,Subset_input,Type_of_arrangement,Type_of_arrangement_input):
	...
	set_count = input('Enter number of sets to be generated')
	sets_list=[]
	...
	if set_count==2:
		...
		sets_list.append(words_list.pop())
		sets_list.append(numbers_list.pop())
		for i in list(np.random.choice(ind_set_count3,3)):
			ind_set.append(i)
		temp=random.random()
	...
	#("Type - 1 : Straight\nType - 2 : Alternatively\nType - 3 : No particular order\n")
	type_of_arrangement = np.random.choice([1,2],1,p=[0.8,0.2])
